  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // silently fail — home screen degrades gracefully to empty state
      } finally {
      ...
      <ScrollView style={s.container} showsVerticalScrollIndicator={false} contentContainerStyle={s.content}>
        ...
        <View style={s.header}>
          ...
          <View style={s.headerActions}>
            ...
            <TouchableOpacity style={s.iconBtn} onPress={toggleTheme} activeOpacity={0.7}>
              <Ionicons name={isDarkMode ? 'sunny' : 'moon'} size={20} color={theme.primary} />
            </TouchableOpacity>
            {isAdmin && (
              <TouchableOpacity style={s.iconBtn} onPress={() => nav.navigate('AdminStack')}>
        ...
        <View style={s.section}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.catScroll}>
            {CATEGORIES.map((cat) => (
              <TouchableOpacity key={cat.id} style={s.catItem} onPress={() => nav.navigate('Browse', { initialCategory: cat.id })}>
        ...
        <TouchableOpacity activeOpacity={0.9} onPress={() => nav.navigate('Upload')} style={s.uploadBannerWrap}>
          <LinearGradient
            colors={isDarkMode ? ['#1e3a5f', '#1e2d5a'] : ['#e0f2fe', '#bae6fd']}
...
const getStyles = (t) => StyleSheet.create({
  ...
  greeting: { fontSize: 24, color: t.text, fontWeight: '800', letterSpacing: -0.5, marginBottom: 4 },
  dateText: { fontSize: 14, color: t.textSecondary, fontWeight: '500' },
  headerActions: { flexDirection: 'row', alignItems: 'center', gap: 8 },
  iconBtn: { width: 44, height: 44, borderRadius: 22, backgroundColor: t.card, justifyContent: 'center', alignItems: 'center', borderWidth: 1, borderColor: t.border },
  adminBtn: { width: 44, height: 44, borderRadius: 22, backgroundColor: t.card, justifyContent: 'center', alignItems: 'center', borderWidth: 1, borderColor: t.border },

  heroCard: { borderRadius: 28, marginBottom: 32, shadowColor: t.accent, shadowOpacity: 0.3, shadowRadius: 16, shadowOffset: { width: 0, height: 8 }, elevation: 8 },
  ...
  badgeText: { fontSize: 10, fontWeight: '800', color: t.accent, letterSpacing: 1 },
  ...
  sectionTitle: { fontSize: 20, fontWeight: '800', color: t.text, letterSpacing: -0.3 },
  seeAllText: { color: t.primary, fontWeight: '600', fontSize: 15 },
  
  catScroll: { gap: 16, paddingRight: 20 },
  catItem: { alignItems: 'center', width: 72 },
  catIconWrap: { width: 64, height: 64, borderRadius: 32, backgroundColor: t.card, justifyContent: 'center', alignItems: 'center', marginBottom: 8, shadowColor: '#1a1a1a', shadowOpacity: 0.05, shadowRadius: 10, elevation: 2 },
  catEmoji: { fontSize: 28 },
  catName: { fontSize: 12, fontWeight: '600', color: t.textSecondary, textAlign: 'center' },

  uploadBannerWrap: { marginBottom: 32, shadowColor: t.primary, shadowOpacity: 0.12, shadowRadius: 10, shadowOffset: { width: 0, height: 4 }, elevation: 2 },
  uploadBanner: { flexDirection: 'row', alignItems: 'center', padding: 20, borderRadius: 24, borderWidth: 1, borderColor: t.glassBorder },
  uploadIconBox: { width: 48, height: 48, borderRadius: 24, backgroundColor: t.glassBg, justifyContent: 'center', alignItems: 'center' },
  uploadTitle: { fontSize: 15, fontWeight: '700', color: t.text, marginBottom: 4 },
  uploadSub: { fontSize: 13, color: t.textSecondary, fontWeight: '500' },

  featScroll: { gap: 16, paddingRight: 20 },
  featCard: { width: FEATURED_CARD_WIDTH, backgroundColor: t.card, borderRadius: 20, shadowColor: '#1a1a1a', shadowOpacity: 0.04, shadowRadius: 10, elevation: 2, overflow: 'hidden' },
  featImgWrap: { width: '100%', height: FEATURED_CARD_WIDTH, backgroundColor: t.background },
  featImg: { width: '100%', height: '100%' },
  featImgPH: { flex: 1, justifyContent: 'center', alignItems: 'center' },
  featBody: { padding: 12 },
  featName: { fontSize: 13, fontWeight: '700', color: t.text, marginBottom: 4 },
  featPrice: { fontSize: 13, color: t.primary, fontWeight: '700' },

  emptyText: { color: t.icon, fontSize: 15, fontStyle: 'italic' },
});
